...
// %FILECHECK% PREFIX_COMBINE ptx,cas_subword_bitwise
// %FILECHECK% PREFIX_COMBINE nvvm,cas_subword_bitwise
...
; PTX_CAS_SUBWORD_BITWISE-DAG: {{.*}}ISETP.NE{{.*}} [[OLD]], [[EXPECTED]], {{.*}}
; NVVM_CAS_SUBWORD_BITWISE-DAG: {{.*}}ISETP.NE{{.*}} [[EXPECTED]], [[OLD]], {{.*}}
